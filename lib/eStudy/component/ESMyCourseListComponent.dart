      ...
      itemBuilder: (BuildContext context, int i) {
        return Container(
          ...
          child: Column(
            ...
            children: [
              cachedImage(list_data[i].image.validate(),
              ...
              widget.index == 0
                  ? Row(
                      children: [
                        Text('56%', style: secondaryTextStyle(size: 12))
                            .expand(flex: 1),
                        LinearProgressIndicator(
                          backgroundColor: ESPrimaryColor,
                          valueColor: AlwaysStoppedAnimation<Color>(
                              ESPrimaryColor.withOpacity(0.2)),
                          value: 56,
                        ).expand(flex: 2),
                        Icon(Icons.star, color: Colors.orange, size: 16)
                            .expand(flex: 1),
                        Text('(4.5)', style: secondaryTextStyle(size: 12))
                            .expand(flex: 1),
                      ],
                    )
                  : AppButton(
                      ...
                      shapeBorder: RoundedRectangleBorder(
                          borderRadius: radius(8),
                          side: BorderSide(color: ESPrimaryColor)),
                      child: Text('View Certificate',
                          style: secondaryTextStyle(color: ESPrimaryColor)),
                    ),
              8.height,
            ],
          ),
          ...
          () {
            ESCourseScreen(name: list_data[i].title, img: list_data[i].image)
                .launch(context);
          },
        );
      },
