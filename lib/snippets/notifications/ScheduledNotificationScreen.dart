...
class ScheduledNotificationScreen extends StatefulWidget {
  ...
  @override
  ScheduledNotificationScreenState createState() =>
      ScheduledNotificationScreenState();
}

class ScheduledNotificationScreenState
              ...
              AppButton(
                ...
                onTap: () async {
                  if (await pickScheduleDate(context)) {
                    toast(
                        'Notification Scheduled at ${DateFormat('hh:mm a').format(_pickedDate!)}');
                ...
                builder: (_, snap) {
                  if (snap.hasData) {
                    if (snap.data!.isNotEmpty) {
                        ...
                        itemBuilder: (_, index) {
                          ...
                          return Container(
                            child: Column(
                              ...
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(data.content!.title.validate(),
                                    style: boldTextStyle()),
                                4.height,
                                Text(
                                    DateFormat('dd/MM/yyyy hh:mm a').format(
                                        DateTime.parse(
                                            "${data.content!.displayedDate!}")),
                                    style: secondaryTextStyle()),
                              ],
                            ),
                            padding: EdgeInsets.all(8),
                      ...
                      return Text('No data', style: primaryTextStyle())
                          .center();
                    }
                  }
